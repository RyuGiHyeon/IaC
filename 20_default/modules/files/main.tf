resource "local_file" "db_script" {
  content  = <<-EOF
    ...
    mariadb -h ${var.rds_address} -u root -p${var.password} < /tmp/${var.file_map.sql.key}
    sleep 6
    shutdown -P now
    EOF
  filename = "${path.root}/files/${var.file_map.db_script.key}"
}
...
resource "local_file" "web_script" {
  content  = <<-EOF
    ...
    EOF
  filename = "${path.root}/files/${var.file_map.web_script.key}"
}
...
resource "local_file" "sql" {
  content  = <<-EOF
    ...
    EOF
  filename = "${path.root}/files/${var.file_map.sql.key}"
}
...
resource "local_file" "config" {
  content  = <<-EOF
    ...
    ?>
    EOF
  filename = "${path.root}/files/${var.file_map.config.key}"
}
...
php_value[session.save_path]    = "tls://${var.cache_endpoint.address}:${var.cache_endpoint.port}?cluster=1"
  EOF
  filename = "${path.root}/files/${var.file_map.www_conf.key}"
}
