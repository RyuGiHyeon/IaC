...
resource "aws_subnet" "privates" {
  ...
  vpc_id            = aws_vpc.main.id
  cidr_block        = cidrsubnet(var.cidr_block, var.subnet_bit, count.index + var.count_of_public_subnets)
  availability_zone = local.az_names[count.index % var.count_of_az]
  tags = {
...
resource "aws_security_group" "rules" {
  count = length(var.fw-rules)

  name        = var.fw-rules[count.index].name
  description = var.fw-rules[count.index].description
  vpc_id      = aws_vpc.main.id
  ingress {
    protocol    = var.fw-rules[count.index].in.protocol
    from_port   = var.fw-rules[count.index].in.from_port
    to_port     = var.fw-rules[count.index].in.to_port
    cidr_blocks = var.fw-rules[count.index].in.cidr_blocks
  }
  egress {
    protocol    = var.fw-rules[count.index].out.protocol
    from_port   = var.fw-rules[count.index].out.from_port
    to_port     = var.fw-rules[count.index].out.to_port
    cidr_blocks = var.fw-rules[count.index].out.cidr_blocks
  }
  tags = {
    Name = var.fw-rules[count.index].name
    key = var.fw-rules[count.index].key
  }
}
